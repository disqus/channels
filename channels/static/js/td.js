(function() {
  ...
  $(document).ready(function() {
    ...
    return $.getScript(realtime_host + '/socket.io/socket.io.js').done(function(script, status) {
      ...
      socket.on('peer_connect', function(peer) {
        var u;
        u = new User(peer);
        return ap_view.addUser(u);
      });
      return socket.on('admin_message', function(message) {
        post = new Post({
          name: "System Message",
          avatar: "/static/img/disqus_avatar.png",
          "class": "system",
          hash: "d0514f5b74496ddc7a07af22103cd116",
          message: message
        });
        return list_view.addPost(post);
      });
    });
  });
